...
class FetchProvinceCovid extends HTMLElement {
  ...
  _renderItems () {
    ...
    for (const key in this.data) {
      const { denominazione_provincia, totale_casi } = this.data[key]
      html += `<item-prov-covid city="${denominazione_provincia}" casi="${totale_casi}"></item-prov-covid>`
    }
...
window.addEventListener('DOMContentLoaded', function () {
  this.customElements.define('fetch-province-covid', FetchProvinceCovid)
})
